		...
		for (;;) {
			try {
				...
				for (UserHostRelation host : listStr) {
					...
					byte[] msg = ToHexUtil.hexStringToByte(CmdUtil.connectService(mAesUtil, host.getHostMac()));
					session.write(IoBuffer.wrap(msg));

					byte[] msg1 = ToHexUtil.hexStringToByte(CmdUtil.getToService(mAesUtil, getDevice(), host.getHostMac()));
					session.write(IoBuffer.wrap(msg1));

					byte[] msg5 = ToHexUtil.hexStringToByte(CmdUtil.getToService(mAesUtil, getEndPoint(), host.getHostMac()));
